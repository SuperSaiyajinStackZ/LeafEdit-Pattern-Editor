...
static void DrawTop(uint Selection, std::vector<DirEntry> dirContents, std::string msg) {
	std::string files;
	Gui::clearTextBufs();
	...
	Gui::DrawStringCentered(0, -2, 0.9f, C2D_Color32(255, 255, 255, 255), msg, 390, 0, fnt);
...
bool Overlays::SelectFile(const std::string &path, std::vector<std::string> extensions, std::string &file, std::string message, bool allowSub) {
	s32 selectedFile = 0;
	...
	while (1) {
		/* Screen draw part. */
		DrawTop(selectedFile, dirContents, message);
		DrawBottom();
