...
std::unique_ptr<Sav> Sav::getSave(std::shared_ptr<u8[]> dt, u32 length, std::string Loc) {
	switch (length) {
		case 0x40000:
		case 0x4007A:
		case 0x8007A:
			...
			if (memcmp(dt.get(), dt.get() + 0x12224, 0x12224) == 0) {
				return std::make_unique<SavWW>(dt, WWRegion::JPN_REV0, length, Loc);
			} else if (memcmp(dt.get(), dt.get() + 0x15FE0, 0x15FE0) == 0) {
				return std::make_unique<SavWW>(dt, WWRegion::EUR_REV1, length, Loc);
			} else if (memcmp(dt.get(), dt.get() + 0x173FC, 0x173FC) == 0) {
				return std::make_unique<SavWW>(dt, WWRegion::KOR_REV1, length, Loc);
			} else {
				return nullptr;
			}
		...
		case 0x7FA00:
			...
			if (memcmp(dt.get() + 0x80, ACNLArray, 0x1C) == 0) {
				return std::make_unique<SavNL>(dt, length, Loc);
			} else {
				return nullptr;
			}

		case 0x80000:
			return check080000(dt, length, Loc);

		case 0x89B00:
				// Check if save header matches first! We don't want to edit a non-ac save.
			if (memcmp(dt.get() + 0x80, ACWAArray, 0x1C) == 0) {
				return std::make_unique<SavWA>(dt, length, Loc);
			} else {
				return nullptr;
			}
...
std::unique_ptr<Sav> Sav::check080000(std::shared_ptr<u8[]> dt, u32 length, std::string Loc) {
